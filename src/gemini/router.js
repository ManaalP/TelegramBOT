...
function rulesQuery(text) {
  ...
  if (dynamicDays) {
    time = `newer_than:${parseInt(dynamicDays[1])}d`;
  } else if (dynamicMonths) {
    time = `newer_than:${parseInt(dynamicMonths[1]) * 30}d`;
  } else if (dynamicYears) {
    time = `newer_than:${parseInt(dynamicYears[1]) * 365}d`;
  } else if (/\b(today|24\s*h(ours?)?|last\s*24)\b/.test(t)) time = "newer_than:1d";
  else if (/\b(48\s*h(ours?)?|last\s*48|yesterday)\b/.test(t)) time = "newer_than:2d";
  else if (/\b(week|last\s*week)\b/.test(t)) time = "newer_than:7d";
  else if (/\b(month|this\s*month|last\s*month)\b/.test(t)) time = "newer_than:30d";
  else if (/\b(quarter)\b/.test(t)) time = "newer_than:90d";
  else if (/\b(half\s*year)\b/.test(t)) time = "newer_than:180d";
  else if (/\b(year|last\s*year)\b/.test(t)) time = "newer_than:365d";
